...
//
const BotName = 'MR~JOHN'; // Nama Bot Whatsapp
const instagramlu = 'https://instagram.com/kang_p_wkwkwk/'; // Nama Instagramlu cok
const whatsapplu = '6281905288936'; // Nomor whatsapplu cok
const kapanbotaktif = 'SEMAU GW NGIDUPINNYA KAPAN'; // Kapan bot lu aktif
const grupch1 = 'BELOM ADA COK'; // OFFICIAL GRUP LU 1
const grupch2 = 'BELOM ADA COK'; // OFFICIAL GRUP LU 2
//
const
{
   WAConnection,
...
conn.on('user-presence-update', json => console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @MR_HADI`))
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @MR_HADI`)
})
...
axios.get(`https://id.m.wikipedia.org/wiki/${teks}`).then((res) => {
...
conn.sendMessage(id, menu.menu(id, BotName, corohelp, tampilTanggal, tampilWaktu, instagramlu, whatsapplu, kapanbotaktif, grupch1, grupch2) ,MessageType.text);
	...
	conn.sendMessage(id,'Admin: 628990911211\nhttps://wa.me/628990911211',MessageType.text);
}
	...
	conn.sendMessage(id,'*'+tampilWaktu+' SISTEM RESTART*',MessageType.text);
	}
else if (text == '#quran'){
axios.get('https://api.banghasan.com/quran/format/json/acak').then((res) => {
	...
	conn.sendMessage(id,'_error command #menu not found try: #info_',MessageType.text);}
else if (text == '#info'){
  const corohelp = await get.get('https://covid19.mathdro.id/api/countries/id').json()
var date = new Date();
var tahun = date.getFullYear();
...
conn.sendMessage(id, 'kirim #ptl cewek/cowok\n\nContoh: #ptl cewek' ,MessageType.text);
   ...
   if (messageType == 'imageMessage')
   {
      ...
      if (caption == '#sticker')
      {
         conn.sendMessage(id, '_Diproses_' ,MessageType.text);
...
/*   if (text.includes("#yt"))
   ...
   if (text == '#waifu'){
    ...
    var oli = items[Math.floor(Math.random() * items.length)];
    var url = "https://api.fdci.se/rep.php?gambar=" + oli;
    
    axios.get(url)
      .then((result) => {
        var b = JSON.parse(JSON.stringify(result.data));
        var oli =  b[Math.floor(Math.random() * b.length)];
        imageToBase64(oli) // Path to the image
        .then(
            (response) => {
  var buf = Buffer.from(response, 'base64'); // Ta-da 
              conn.sendMessage(
